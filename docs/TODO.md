...
| T-049 | SNS投稿アイディア蓄積・半自動作成ワークフロー | L | プロセス | 対話中にDTが指定したSNS投稿ネタをJSONで蓄積し、ワークフローに基づいて下書き生成→承認→投稿用テキスト出力する仕組み。蓄積: Chat対話中に `sns-ideas.json` へ追記push。作成: 指示書でDT Codeに下書き生成させるか、Chat側で直接生成。承認: DTがスマホで確認→投稿 |
