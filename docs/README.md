...
負荷カテゴリ判定（下表）
  ↓
[低] → そのまま着手
[中] → ユーザーに作業範囲を確認してから着手
[高] → セッション分割計画を提示し、ユーザー承認を得てから着手
```
...
| 🔴 高 | 複数シェーダー修正、Gemini MCP連携、4エージェント分析、新機能の設計+実装 | **セッション分割を提案** |

**🔴高の具体例と分割パターン:**

- 「渦シェーダーを修正して」→ Session A: 要件整理+Geminiプロンプト設計 / Session B: 実装+統合
- 「h1が暗い原因を調べて直して」→ Session A: 原因調査（最小限のファイル参照） / Session B: 修正実装
- 「サイト全体をレビューして」→ Session A: 構造分析 / Session B: 改善実装
...
セッションヘルスガードの実効性は以下の**3層**で担保する:

| 層 | 仕組み | 役割 |
|----|--------|------|
| **Memory** | Claudeの永続記憶にSH-1〜SH-6の要約を記録 | セッションをまたいでも消えない。ガードの存在を想起させる |
| **README §2 Step 1.5** | セッション開始手順に負荷事前判定を組み込み | タスク着手前の強制チェックポイント |
| **README §12（本節）** | 詳細な監視項目・閾値・対応策 | Step 1.5で🔴高と判定された場合の参照先 |

**Memory → Step 1.5 → §12** の順に参照が深くなる。Memoryが最初のトリガーとなり、「README §12を読め」と指示する。
...
以下のタイミングで自動チェック:
- **タスク受領時**: Step 1.5で負荷カテゴリ判定（🟢低/🟡中/🔴高）
- ファイル読み込み前（SH-1, SH-3）
- ツール呼び出し前（SH-4, SH-5）
- 長文出力の開始前（SH-2）
- 4エージェント分析の開始前（SH-6）
