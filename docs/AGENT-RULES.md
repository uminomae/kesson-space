...
| 📋 **プロジェクト管理** | タスク発生 | タスク認識時・指示書作成時 | `.claude/rules/project-management-agent.md` |
...
詳細: `.claude/rules/project-management-agent.md`（Autonomy Charter / State Machine）
...
> **原則**: 外部エージェント利用は `.claude/rules/project-management-agent.md` の自律判断ルールに従う。ユーザーの明示指示がある場合はそれを最優先する。
...
| **L1: スキル** | ロール別ガイドライン（.claude/rules/から選択） | タスクに応じて1-2枚 | ~400/枚 |
...
### L1: スキル（.claude/rules/ から選択ロード）

**1回のプロンプトで読み込むのは1〜2ファイルまで**。全部連結して読ませない。

| スキルファイル | 対象 | 内容 |
|---|---|---|
| `.claude/rules/shared-quality.md` | 全エージェント | 命名規約、禁止事項、パフォーマンス基準 |
| `.claude/rules/shader-impl.md` | Gemini | Visual Direction、GLSL作法、出力ルール |
| `.claude/rules/review-gates.md` | GPT | レビュー観点、品質ゲート、評価基準 |
| `.claude/rules/orchestrator.md` | Claude | 分解・委譲・統合の手順 |
| `.claude/rules/project-management-agent.md` | Claude | 委譲判断・指示書生成 |
| `transform/devlog-generation.md` | Claude/Codex | devlog生成ワークフロー |
...
渡すもの:
  ...
  L1: .claude/rules/shader-impl.md（+ shared-quality.md 必要時）
...
```
渡すもの:
  指示書（.claude/rules/project-management-agent.md のテンプレート）
  ワークツリーパス指定必須
  ブランチ名指定必須
```
...
渡すもの:
  L0: Grounding
  L1: .claude/rules/review-gates.md
...
- **.claude/rules/*.md**: 月1回の棚卸し or 明らかな問題発見時
...
将来的にPROMPT-STRUCTURE.mdの内容は `.claude/rules/shader-impl.md` と `context-pack/TASK.md` テンプレートに分解・吸収される可能性がある。
