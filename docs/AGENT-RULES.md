...
**→ [README.md §12](./README.md#12--セッションヘルスガード) を参照。**

kesson-spaceではシェーダーファイル（GLSL 200行超）やGemini MCP応答の蓄積により、コンテキストウィンドウが逼迫しやすい。🩺セッションヘルスはClaudeが内部的に常時監視するガードであり、以下を自動チェックする:

- SH-1: シェーダー全文読み込み数
- SH-2: 1ターンの出力長
- SH-3: 累積ファイル参照数
- SH-4: 連続ツール呼び出し数
- SH-5: Gemini MCP応答の蓄積
- SH-6: 4エージェント分析前のコンテキスト残量
