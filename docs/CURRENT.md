...
**問題**: #19でセッションヘルスガード（§12）を策定済みだったが、「Claudeが内部的に常時監視」はセッションをまたぐと記憶が失われ、実効性がなかった。kesson-thinking の quality-management.md §2.1.2 を参照し、同一の問題を認識。

**解決策（3層担保）**:

| 層 | 変更 | 効果 |
|----|------|------|
| Memory | SH-1〜SH-6の要約 + 負荷事前判定ルールを永続記憶に追加 | セッションをまたいでもガードの存在を想起 |
| README §2 Step 1.5 | タスク受領時の負荷カテゴリ判定（🟢低/🟡中/🔴高）を手順に組み込み | タスク着手前の強制チェックポイント |
| README §12 | Memory連携仕様と事前判定フローを追記 | 参照構造を明文化 |

**Memory追加内容**:
- `#3`: セッションヘルスガード必須（SH-1〜SH-6要約、§12参照指示）
- `#4`: タスク着手前のコンテキスト負荷事前判定（高負荷→セッション分割）
...
- **NEW**: ガードの実効性はMemory→Step 1.5→§12の3層で担保する
