...
import { createQuantumFieldMaterial, createQuantumFieldMesh } from './shaders/quantum-field.glsl.js';
...
export function updateScene(time) {
    ...
    if (toggles.quantumField) {
        const qu = _quantumFieldMaterial.uniforms;
        qu.uTime.value = time * quantumFieldParams.speed;
        qu.uOpacity.value = quantumFieldParams.opacity;
        qu.uSlitPosition.value = quantumFieldParams.slitPosition;
        qu.uWaveCount.value = quantumFieldParams.waveCount;
        qu.uParticleIntensity.value = quantumFieldParams.particleIntensity;
        _quantumFieldMesh.position.set(quantumFieldParams.posX, quantumFieldParams.posY, quantumFieldParams.posZ);
        _quantumFieldMesh.scale.set(quantumFieldParams.size, quantumFieldParams.size, 1);
    }
}
